...
const (
	...
	maxMessageSize = 8192 // Increased for chat messages with metadata
)
...
func (c *Client) handleWebSocketMessage(wsMsg *models.WSMessage) {
	...
	case models.WSMessageTypeUserJoin:
		...
		c.hub.JoinChatRoom(c.userID, wsMsg.ChatID)

	case models.WSMessageTypeUserLeave:
		// User wants to leave a chat room
		c.hub.LeaveChatRoom(c.userID, wsMsg.ChatID)

	case models.WSMessageTypeRead:
		// Mark message as read
		if readData, ok := wsMsg.Data.(map[string]interface{}); ok {
			if messageID, exists := readData["message_id"].(float64); exists {
				readReceiptData := map[string]interface{}{
					"message_id": uint(messageID),
					"user_id":    c.userID,
					"read_at":    time.Now(),
				}
				c.hub.BroadcastToChatExcludeSender(wsMsg.ChatID, readReceiptData, models.WSMessageTypeRead, c.userID)
			}
		}

	case models.WSMessageTypeReaction:
		// Handle emoji reaction
		if reactionData, ok := wsMsg.Data.(map[string]interface{}); ok {
			reactionData["user_id"] = c.userID
			reactionData["timestamp"] = time.Now()
			c.hub.BroadcastToChatExcludeSender(wsMsg.ChatID, reactionData, models.WSMessageTypeReaction, c.userID)
		}
	...
	case "heartbeat":
		...
		default:
			log.Printf("Could not send heartbeat response to user %d", c.userID)
		}

	default:
		log.Printf("Unknown WebSocket message type from user %d: %s", c.userID, wsMsg.Type)
	}
}
